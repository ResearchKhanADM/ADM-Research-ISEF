...
- **R3's threshold claim is conditional, and the condition is a number.** **R3 holds conditional on `C` being a lagged filter rather than a bistable memory. The boundary is `ε = 1.54`. The pre-registered sweep straddles it. If the plausible range for `ε` sits above 1.54, `C` can hold its own state after withdrawal and re-suppress the acinar program, and R3 inverts.** *(v3.2, decision 015.)* State it unprompted — a reversal condition with a coordinate attached is worth more than any hedge.
